...
AddrSpace::AddrSpace()
{
    if (bitmap ==NULL){
        ...
        //bitmap created with the RAM size to keep track which page is empty
...
// how big is address space?
    ...
    for (int i = 0; i < numPages; i++) {
        ...
        pageTable[i].swapLocation = kernel->getSwapLocCount();
        ...
        if(pageTable[i].physicalPage != -1){
            //copy data into main memory as long aa there are free pages in RAM
            bzero(&kernel->machine->mainMemory[pageTable[i].physicalPage * PageSize], PageSize);
            for(int k = 0, j = 0; j < PageSize ; k++, j++)
            {
                kernel->machine->mainMemory[pageTable[i].physicalPage * PageSize + j] = buffer[k];
            }

            //append the page into FIFO page list
            kernel->PageList->Append(pageTable[i].physicalPage);

            //insert the page and the corresponding process into a map

            if(kernel->pageProcessMap->find(pageTable[i].physicalPage) == kernel->pageProcessMap->end()){    
                kernel->pageProcessMap->insert(pair <int, int> (pageTable[i].physicalPage, kernel->currentThread->getPID()));
            } else {
                kernel->pageProcessMap->find(pageTable[i].physicalPage)->second = kernel->currentThread->getPID();
            }
            
        } else {
            //No free pages in main memory and thus call PageFault exception
            kernel->stats->numPageFaults++;
            pageTable[i].valid = FALSE;  
        }

        //Write into swap file
        kernel->swapFile->WriteAt(buffer, PageSize, pageTable[i].swapLocation * PageSize);
...
//get the entry from the pageTable correspnding to given vpn
TranslationEntry* AddrSpace::getPageTableEntry(int vpn){
