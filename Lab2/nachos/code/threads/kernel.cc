...
Kernel::Initialize()
{
    ...
    PageList = new List<int>();
     //mapping of page table entry to main memory
    pageProcessMap = new map<int,int>;
    ...
    //cout<<"file system1 creation called...\n";
#else
    fileSystem = new FileSystem(formatFlag);
    //cout<<"file system2 creation called...\n";
#endif // FILESYS_STUB
    //create a new swapfile using OpenFIle executable and open it

    //fifo list
    
    bool flag = kernel->fileSystem->Create("New_SwapFile");
    
    if(flag){
        swapFile = kernel->fileSystem->Open("New_SwapFile");    
    }
...
void Kernel::cleanPhysicalMemory(int pid){

    map <int, int> :: iterator iter = pageProcessMap->begin();
    while(iter != pageProcessMap->end()){
        if(iter->second == pid){

            int pageNo = iter->first;
            AddrSpace::bitmap->Clear(pageNo);

            PageList->Remove(pageNo);

            bzero(&kernel->machine->mainMemory[pageNo * PageSize], PageSize);

            TranslationEntry *pageEntry = kernel->currentThread->space->getPageTableEntry(pageNo);

            if(pageEntry != NULL){
                char *buffer = new char[PageSize];
                for(int k = 0, j = 0; j < PageSize ; k++, j++)
                {
                    kernel->machine->mainMemory[pageNo * PageSize + j] = buffer[k];
                }

                //swapFile->WriteAt(buffer, PageSize, pageEntry->swapLocation * PageSize);
            }
