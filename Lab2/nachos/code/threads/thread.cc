...
Thread::Finish ()
{
    ...
    kernel->cleanPhysicalMemory(kernel->currentThread->getPID());
...
Thread::Sleep (bool finishing)
{
    ...
    while ((nextThread = kernel->scheduler->FindNextToRun()) == NULL){
        if(flag == TRUE){
            cout<<"\nTotal number of page faults: " << kernel->stats->numPageFaults << endl;
            flag = FALSE;
        }
...
//-------------------------------------------------------------------------
// get_ThreadId
// Iterate the thread list and return the thread for which the page is getting
// evicted from the PageList during page fault exception
//-------------------------------------------------------------------------

Thread*
Thread::getTID(int pid){
    
    ListIterator<Thread*> *iterator = new ListIterator<Thread*>(kernel->threadList);

    for (; !iterator->IsDone(); ) {
        if (iterator->Item()->getPID() == pid){
