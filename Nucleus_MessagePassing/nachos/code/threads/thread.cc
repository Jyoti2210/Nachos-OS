...
Thread::Thread(char* threadName)
{
    ...
    pid = pid_count;
    pid_count++;
    // cout<<"Thread enters"<<endl;
    kernel->thread_list->Append(this);
    // allThreads->Append(this);
    threadMessageQueue = new map<int, MessageBuffer*>;
    // cout<<"Thread enters"<<endl;
...
Thread::Finish ()
{
    ...
    // cout<<"finished"<<endl;
    kernel->machine->CleanMainMemory(this->GetPid());
    // cout<<"finished"<<endl;
...
Thread::Sleep (bool finishing)
{
    ...
    // cout<<"finished"<<nextThread->GetPid()<<endl;
    // cout<<"test"<<endl;
...
map<int, MessageBuffer*>* 
Thread::GetThreadMessageQueue() {
    // cout << "In process message queue" << endl;
...
Thread*
Thread::GetThreadByName(char* thread_name){
    //cout << thread_name <<  "---------" <<endl;
    // cout << kernel->currentThread->getName() << endl;
    string str1 = thread_name;
    string str2 = "test";
    // cout<<"harshad "<<str1<<endl;
    // cout << "Length " <<kernel->thread_list->NumInList() << endl;
    ListIterator<Thread*> *iterator_b = new ListIterator<Thread*>(kernel->thread_list);

    for (; !iterator_b->IsDone(); iterator_b->Next()){
        // cout<<"for enter " << endl;

        // Thread *temp = iterator_b->Item();

        if(iterator_b->Item()->GetPid() != 12) {
            // cout << temp->getName() << endl;

            string t_name = iterator_b->Item()->getName();

            // cout<<"string convert"<<endl;
            
            if (t_name.find(str1) != string::npos)
            {
                // cout<<"string found"<< t_name << " " << str1 << endl;
                return iterator_b->Item();
            }
        }
...
bool 
Thread::IsProcessExist(int pid) {
    ListIterator<Thread*> *iter = new ListIterator<Thread*>(kernel->thread_list);
    while(!iter->IsDone()) {
        Thread *curr = iter->Item();
        if(curr->GetPid() == pid) {
            return true;
        }
        iter->Next();
...
MessageBuffer* 
Thread::FetchThreadBuffer(int bufferID, char* sender){
    map<int,MessageBuffer*> *messagesQueue = this->GetThreadMessageQueue();
    if(!messagesQueue->empty()){
        // cout<<"enter in if fetchProcessBuffer"<<sender<<endl;
        if(messagesQueue->count(bufferID) != 0){
            // cout<<"count ==0"<<endl;
            return (messagesQueue->find(bufferID))->second;
        }
        else if(sender != NULL)
        {
            // cout<<"enter in if fetchProcessBuffer sneder != null"<<endl;
            map<int, MessageBuffer*> :: iterator it;
            for(it = messagesQueue->begin(); it != messagesQueue->end(); it++){
                // cout<<"Sender"<<sender<<endl;
                string iterator_name = it->second->getSender()->GetName();
                string sender_name = sender;

                if(iterator_name.find(sender_name) != string::npos){
