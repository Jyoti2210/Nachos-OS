...
Bitmap* Kernel::mBuffers = NULL;
map<int, List<MessageBuffer*>*>* Kernel::messageBufferQueue = NULL;
//----------------------------------------------------------------------
// Kernel::Kernel
...
Kernel::Initialize()
{
    ...
    messageBufferQueue = new map<int, List<MessageBuffer*>*>;
    // hsd
    // cout<<" enters"<<endl;
    stats = new Statistics();		// collect statistics
    interrupt = new Interrupt;		// start up interrupt handling
    scheduler = new Scheduler();	// initialize the ready queue
    alarm = new Alarm(randomSlice, quantum_timer);	// start up time slicing
    ...
    //hsd
    // cout<<" enters"<<endl;
    bool flag_file = fileSystem->Create("SwapFile");
    if (flag_file){
        swapFile = fileSystem->Open("SwapFile");
    }
    // cout<<" enters"<<endl;
    page_table_map = new map<int, int>;

    list_fifo = new List<int>;
    //hsd
...
Bitmap* 
Kernel::getMessageBuffer() {
    return mBuffers;
}

map<int,List<MessageBuffer*>*>* 
Kernel::getMessageBufferQueue() {
    return messageBufferQueue;
}

void 
Kernel::ClearKernelBufferQ(int bufferID) {
    getMessageBuffer()->Clear(bufferID);
    if(getMessageBufferQueue()->count(bufferID) != 0) {
        // cout << "Clearing the Message buffer: " << bufferID;
        if(!getMessageBufferQueue()->empty())
            getMessageBufferQueue()->erase(bufferID);
    }
}

int 
Kernel::getMessageLimit(){
...
MessageBuffer* 
Kernel::ValidateReceiver(int bufferID, Thread* receiverProcess){
    MessageBuffer *mBuffer = kernel->getMessageBufferQueue()->find(bufferID)->second->Front();
    if(mBuffer != NULL){
