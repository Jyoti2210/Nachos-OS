...
void
thread_met(char *filename)
{
    // cout<<"thread_met enters"<<endl;
    RunUserProg(filename);
}

void ProgThread(char *filename){
    Thread *t = new Thread(filename);
    t->Fork((VoidFunctionPtr) thread_met, (char *) filename);
}
...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        // cout<<"prog name "<< userProgName.substr(0, 3) <<endl;
        thread_vec.push_back(userProgName);    //vinu
        i++;
        // cout<<"-x Exit"<<endl;
	}
    ...
    //     // cout<<"if loop exit  "<<endl;
    //   RunUserProg(userProgName);
    // }

    for(int i=0; i < thread_vec.size(); i++){
        // cout<<"for loop enter  "<<endl;
        ProgThread(thread_vec[i]);
