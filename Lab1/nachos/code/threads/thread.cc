...
Thread::Fork(Process* p,VoidFunctionPtr func, void *arg)
{
    ...
    Scheduler1 *scheduler = kernel->currentProcess->scheduler;   
    IntStatus oldLevel;
    ...
    p->scheduler->ReadyToRunThread(this);	// ReadyToRun assumes that interrupts 
					// are disabled!
...
{
    cout<<"thread fork::thread forked"<<endl;
    Interrupt *interrupt = kernel->interrupt;
    Scheduler1 *scheduler = kernel->currentProcess->scheduler;   
    IntStatus oldLevel;
    ...
    oldLevel = interrupt->SetLevel(IntOff);
    kernel->currentProcess->scheduler->ReadyToRunThread(this);   // ReadyToRun assumes that interrupts 
                    // are disabled!
    (void) interrupt->SetLevel(oldLevel);
}  
...
Thread::Yield ()
{
    ...
    nextThread = kernel->currentProcess->scheduler->FindNextThreadToRun();
    if (nextThread != NULL) {
    	kernel->currentProcess->scheduler->ReadyToRunThread(this);
    	kernel->currentProcess->scheduler->RunThread(nextThread, FALSE);
    }
...
Thread::Sleep (bool finishing)
{
    ...
    status = BLOCKED;
    while ((nextThread = kernel->currentProcess->scheduler->FindNextThreadToRun()) == NULL){ 
        cout<<endl;
        cout<<kernel->currentProcess->getName()<<" Finished here!!" <<endl;
        cout<<"-----------------------------------------------------------\n";
        kernel->currentProcess->Finish();   // no one to run, wait for an interrupt   

    }
	
    // returns when it's time for us to run
    
    kernel->currentProcess->scheduler->RunThread(nextThread, finishing); 
}
...
SimpleThread(int which)
{
	...
	cout << "*** thread " << which << " looped " << num << " times\n";
        cout<<"yield is called in thread simplethread"<<endl;
...
Thread::SelfTest()
{
    ...
    cout<<"yield is called in thread self test"<<endl;
    kernel->currentProcess->currentThread->Yield();
    SimpleThread(0);
}
