...
Semaphore::P()
{
    ...
    Process *currentProcess = kernel->currentProcess;
    
    // disable interrupts
    IntStatus oldLevel = interrupt->SetLevel(IntOff);	
    
    while (value == 0) { 		// semaphore not available
    	queue->Append(currentProcess);	// so go to sleep
    	currentProcess->Sleep(FALSE);
    } 
...
Semaphore::V()
{
	...
	kernel->scheduler->ReadyToRunProcess(queue->RemoveFront());
    }
    value++;
...
{
    semaphore->P();
    lockHolder = kernel->currentProcess;
}
